            ...
            mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                ...
                public void onComplete(@NonNull Task<AuthResult> task) {
                    if (task.isSuccessful()) {
                        ...
                        reference = FirebaseDatabase.getInstance().getReference().child(RegisterActivity.RIDER_USERS).child(username);
                        HashMap<String, String> hashMap = new HashMap<>();
                        hashMap.put("userId", userId);
                        hashMap.put("userName", username);
                        hashMap.put("email", email);
                        hashMap.put("userImage", DEFAULT_IMAGE);
                        hashMap.put("phone", phone);
                        hashMap.put("userAge", "");
                        hashMap.put("userHeight", "");
                        hashMap.put("userAddress", "");
                        hashMap.put("userReligion", "");
                        UserModel model = new UserModel();
                        model.setUserId(userId);
                        model.setUserName(username);
                        model.setUserEmail(email);
                        model.setUserImage(DEFAULT_IMAGE);
                        model.setUserPhone(phone);
                        model.setUserAge("");
                        model.setUserHeight("");
                        model.setUserAddress("");
                        model.setUserReligion("");
                        model.setUserWeight("");
                        model.setUserHeight("");
                        model.setUserDob("");
                        model.setUserComplexion("");
                        model.setLatestEducation("");
                        model.setUniversity("");
                        model.setFatherName("");
                        model.setFatherOccupation("");
                        model.setMotherName("");
                        model.setMotherOccupation("");
                        model.setNoOfSiblings("");
                        model.setAddress("");
                        model.setReligion("");
                        model.setIncome("");*/
